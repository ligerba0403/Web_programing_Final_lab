...
if (!$project) {
    $_SESSION['flash'] = ['type' => 'error', 'msg' => 'Proje bulunamadı.'];
    header('Location: dashboard.php');
    exit;
}

$errors = [];
$data   = $project; // pre-fill form
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare(
            'UPDATE projects SET title=?, title_en=?, description=?, description_en=?, image_url=?,
             technologies=?, category=?, project_url=?, github_url=?, is_featured=?, sort_order=? WHERE id=?'
        );
        $stmt->execute([
        ...
        $_SESSION['flash'] = ['type' => 'success', 'msg' => 'Proje güncellendi.'];
        header('Location: dashboard.php');
        exit;
    }
}
...
<body>
  ...
  <main class="admin-main">
    <header class="admin-header">
      <h1 data-admin-i18n="edit_title">Proje Düzenle <span class="text-muted">#<?= $id ?></span></h1>
      ...
      <form method="POST" action="">
        ...
        <div class="form-actions">
          ...
          <button type="submit" class="btn btn--primary">
            <i data-lucide="check"></i> Güncelle
          </button>
        </div>
      </form>
    </div>
